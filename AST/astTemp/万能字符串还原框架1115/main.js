...
function isObSpFunction(path) {

  let { id, body, params } = path.node;

  if (params.length < 2) {//重复定义的函数，其形参长度全部是5(如果是其他长度，需要更改)，这也是ob混淆的特征
    return false;
  }
  body = body.body;
...
const collectAllSimplieFunc =
{
  FunctionDeclaration(path) {
    ...
    scope.traverse(scope.block, {//在scope内收集符合ob混淆特征的解密函数
      "FunctionDeclaration"(path) {
        let { id, body, params } = path.node;
        if (!isObSpFunction(path)) {
          return;
        }
        decryptCode += path.toString();

        funcNames.push(id.name);//没有做同名函数处理。如果有同名函数，请先将名字差异化。
      },
    })
  },
}
